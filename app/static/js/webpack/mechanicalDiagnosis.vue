  ...
  <div class="mechanical-diagnosis">
    ...
    <div class="content">
      <el-form ref="form" class="mechanical-handle">
        ...
        <el-form-item :label="$t('image')">
          <el-upload :show-file-list="false" :action="upload.url" :on-success="handleSuccess" :file-list="upload.photographList" :data="upload.data">
            <div>
          ...
          <el-row v-if="fileList.img.length > 0">
            <div v-for="(item, index) in fileList.img" :key="index" class="img-holder">
        ...
        <el-form-item :label="$t('video')">
          ...
          <el-row v-if="fileList.video.length > 0">
            <div v-for="(item, index) in fileList.video" :key="index" class="video-holder">
    ...
    <div class="content">
      <el-form class="defect-list">
        <el-form-item v-for="(item, index) in defectOptionList" :key="index">
          <h3 style="color: #e26829; font-size: 20px; text-indent: 18px">
            {{ item.model }}
          </h3>
          ...
          <el-checkbox-group v-model="selectedList[item.type]" class="error-details">
            <el-checkbox v-for="select in item.data" :key="select.name" :label="select.label" @change="changeRecord()"></el-checkbox>
          </el-checkbox-group>
        </el-form-item>
      </el-form>
    </div>
    <div class="content">
      <el-form class="error-list">
        <h3 style="color: #e26829; font-size: 20px; text-indent: 18px">
          {{ errorCodeOptionList.model }}
        </h3>
        <el-checkbox-group v-model="errorCodeList" class="error-details" @change="changeCheckBox">
          <el-checkbox v-for="(error, idx) in errorCodeOptionList.data" :key="error.label + idx" :label="error.label"></el-checkbox>
        </el-checkbox-group>
      </el-form>
    </div>
...
export default {
  ...
  methods: {
    ...
    initErrorCode() {
      this.errorCodeOptionList.data = []
      const lang = Core.Data.getLang()
      ErrorService.findAllGroupByCode().then((res) => {
        for (let i in res.data) {
          const errList = res.data[i]
          const errItem = errList.find((err) => err.language == lang)
          this.errorCodeOptionList.data.push({
            value: errItem,
            label: errItem.name
          })
        }
...
// WEBPACK FOOTER //
// mechanicalDiagnosis.vue?2646f110