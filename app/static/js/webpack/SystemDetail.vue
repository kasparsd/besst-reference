  ...
  <div class="system-detail">
    ...
    <el-form :inline="true" class="demo-form-inline">
      ...
      <el-form-item label="项目类型:">
        <el-select v-model="projectType" placeholder="请选择" @change="getProjectType">
    ...
    handleSystemInfo(type, data, systemItem, typeList, typeHeaderList, nextItem) {
      if (type == 'property') {
        data.forEach((childItem) => {
          ...
          ;(propertyHeadItem.name = childItem.property_name), (propertyHeadItem.label = childItem.property_name)
                  ...
                  if (i == 'hardware') {
                    if (item.paramBodyList[i] && item.paramBodyList[i].length > 0) {
                      this.handleSystemInfo(
                        i,
                        item.paramBodyList[i],
                        systemItem,
                        systemItem.hardwareData,
                        systemItem.hardwareHeadData,
                        hardwareNextItem
                      )
                    }
                  } else if (i == 'firmware') {
                    if (item.paramBodyList[i] && item.paramBodyList[i].length > 0) {
                      this.handleSystemInfo(
                        ...
                        item.paramBodyList[i],
                        systemItem,
                        systemItem.firmwareData,
                        systemItem.firmwareHeadData,
                        firmwareNextItem
                      )
                    }
                  } else if (i == 'property') {
                    if (item.paramBodyList[i] && item.paramBodyList[i].length > 0) {
                      this.handleSystemInfo(
                        ...
                        item.paramBodyList[i],
                        systemItem,
                        systemItem.propertyData,
                        systemItem.propertyHeadData,
                        propertyNextItem
                      )
                    }
                  }
