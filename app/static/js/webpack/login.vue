  ...
  <div class="login">
    ...
    <div class="main-container">
      ...
      <div class="main-warp" @keyup.13="login">
        ...
        <div class="end-porint">
          <el-select v-model="currentEndPoint" @change="changeEndPoint">
            <el-option v-for="(item, index) in endPointList" :key="index" :label="item.label" :value="item.label"> </el-option>
          </el-select>
        </div>
        <el-input type="text" class="username" :placeholder="$t('userNamePlaceholder')" v-model="userName"></el-input>
        <el-input type="password" class="password" :placeholder="$t('passwordPlaceholder')" v-model="password"></el-input>
        <div class="check-div">
          ...
          <div class="item">
            ...
            <div class="overflow">
              <div class="declareInfo">
                ...
                <div class="special">
                  <p><span></span>{{ $t('declareP2') }}</p>
                  <img src="../assets/images/login/besst-tool.png" title="BESST TOOL" />
                </div>
                <br />
                <h3>{{ $t('declareP6') }}</h3>
                <P
                  ><span>{{ $t('declareP7') }}</span></P
                >
                <P
                  ><span>{{ $t('declareP8') }}</span></P
                >
                <P
                  ><span>{{ $t('declareP9') }}</span></P
                >
                <P
                  ><span>{{ $t('declareP10') }}</span></P
                >
                <br />
                <P
                  ><span>{{ $t('declareP11') }}</span></P
                >
                <P
                  ><span>{{ $t('declareP12') }}</span></P
                >
                <P
                  ><span>{{ $t('declareP13') }}</span></P
                >
                <P
                  ><span>{{ $t('declareP14') }}</span></P
                >
                <P
                  ><span>{{ $t('declareP15') }}</span></P
                >
                <P
                  ><span>{{ $t('declareP16') }}</span></P
                >
                <P
                  ><span>{{ $t('declareP17') }}</span></P
                >
                <P
                  ><span></span>
          ...
          <div class="item">
            ...
            <div class="overflow">
              <div class="declareInfo">
                <h3>{{ $t('declareP30') }}</h3>
                <P
                  ><span>{{ $t('declareP31') }}</span></P
                >
                <P
                  ><span>{{ $t('declareP21') }}</span></P
                >
                <P
                  ><span>{{ $t('declareP22') }}</span></P
                >
                <P
                  ><span>{{ $t('declareP23') }}</span></P
                >
                <P
                  ><span>{{ $t('declareP32') }}</span></P
                >
                <br />
                ...
                >
                <P
                  ><span>{{ $t('declareP33') }}</span></P
                >
                <P
                  ><span>{{ $t('declareP34') }}</span></P
                >
                <P
                  ><span>{{ $t('declareP16') }}</span></P
                >
                <P><span></span>{{ $t('declareP17') }} </P>
                <P
                  ><span></span>
                  <a class="btn-underline" href="https://www.bafang-e.com/en/oem-area/service/besst/" target="_blank">
                    https://www.bafang-e.com/en/oem-area/service/besst/</a
          ...
          <div class="item last">
            ...
            <div class="overflow">
              <div class="declareInfo">
                ...
                >
                <P
                  ><span>{{ $t('declareP26') }}</span></P
                >
                <P
                  ><span>{{ $t('declareP27') }}</span></P
                >
                <P
                  ><span>{{ $t('declareP28') }}</span></P
                >
                <P
                  ><span>{{ $t('declareP16') }}</span></P
                >
...
export default {
  ...
  data() {
    return {
      ...
      endPointList: [
        {
          label: '阿里云',
          endPoint: 'http://test.api.besst.bafang-service.com/client/1',
          fileEndPoint: 'http://test.api.besst.bafang-service.com/file/file-upload'
        },
        {
          label: 'EU-Central',
          endPoint: 'http://eu-central-1.api.besst.bafang-service.com/client/1',
          fileEndPoint: 'http://eu-central-1.api.besst.bafang-service.com/file/file-upload'
        },
        {
          label: '体验版',
          endPoint: 'http://dev.acey.cn:9898/client/1',
          fileEndPoint: 'http://dev.acey.cn:9898/file/file-upload'
        },
        // {
        //   label: '本地测试',
        //   endPoint: 'http://192.168.1.218:9898/client/1',
        //   fileEndPoint: 'http://dev.acey.cn:9898/file/file-upload'
        // }
        // {
        //   label: '本地调试',
        //   value: 'http://localhost:9898/client/1'
        // },
      ],
      currentEndPoint: '阿里云',
      Core: Core
  ...
  mounted() {
    ...
    if (Core.Data.getEndPoint()) {
      try {
        const _endPointLabel = Core.Data.getEndPoint()
        if (!_endPointLabel) return
        this.currentEndPoint = _endPointLabel
        const { endPoint, fileEndPoint } = this.endPointList.find((item) => item.label === _endPointLabel)
        Core.Const.NET.END_POINT = endPoint
        Core.Const.NET.FILE_UPLOAD_END_POINT = fileEndPoint
        Core.HttpHelp.http.defaults.baseURL = endPoint
      } catch (error) {
        this.$message.error('请选择正确的环境')
      }
    }
    //            console.log( "md5" );  //
  ...
  methods: {
    ...
    update(p) {
      var request = this.myDB.db.transaction(['person'], 'readwrite').objectStore('person').put({ id: 1, name: 'ps', phone: p })
    ...
    initIp() {
      ...
      //
      //                // 传参并指定回调执行函数为onBack
      //                script.src = 'http://106.14.208.198:6656/ip?callback=onBack';
      ...
      //                $.ajax({
      //                    type:"get",
      //                    url:"http://106.14.208.198:6656/ip",/*url写异域的请求地址*/
      ...
      //                $.getJSON("http://106.14.208.198:6656/ip",function(json){
      //                    console.log(json, 'get complete');
      //                });
      //                this.$http.get('http://106.14.208.198:6656/ip')
      //                    .then((response) => {
    ...
    login() {
      ...
      Core.Api.User.login(this.userName, this.password, this.ip).then((res) => {
        ...
        Core.Data.setEndPoint(this.currentEndPoint)
        Core.HttpHelp.http.defaults.baseURL = Core.Const.NET.END_POINT
    ...
    changeEndPoint(label) {
      const { endPoint, fileEndPoint } = this.endPointList.find((item) => item.label === label)
      Core.Const.NET.END_POINT = endPoint
      Core.Const.NET.FILE_UPLOAD_END_POINT = fileEndPoint
    }
  }
