<template>
  <div class="invitation">
    ...
    <div class="nav-btn">
      <el-button type="primary" icon="arrow-left" @click="$router.back()">{{
        $t("login")
      }}</el-button>
    </div>
    ...
    <el-dialog :visible.sync="accessVisible" size="middle">
      <div>
        <canvas id="mc" width="960" height="480"> 您的浏览器版太低，请升级! </canvas>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    goAccount() {
      ...
      if (this.haveUser) {
        Core.Api.Org.checkInviteByUser(this.code).then((res) => {
          ...
          Core.Data.setRoleEmailType(
            inviteInfo.target_org_email,
            inviteInfo.target_org_type
          );
    ...
    draw() {
      ...
      function drawTextOn(t, x, y, w) {
        ...
        for (var a = 0; a < chr.length; a++) {
          if (ctx.measureText(temp).width < w) {
            ...
            if (
              chr[a].charCodeAt(0) > 128
                ? false
                : chr[a].charCodeAt(0) == 32
                ? false
      ...
      ctx.fillStyle = "#FFF";
      ctx.font =
        '14px  "PingFang SC", "PingFangSC-Semibold", "Helvetica Neue", Helvetica, "Nimbus Sans L", Arial, "Liberation Sans", "Hiragino Sans GB", "Source Han Sans CN", "Source Han Sans SC", "Microsoft YaHei", SimHei, sans-serif';
