          ...
          <tbody v-if="!loading && tableList.length > 0">
            <tr v-for="item in tableList" :key="item.id" class="areaList">
              ...
              <td>
                ...
                <a
                  class="btn-underline"
                  v-ilpermission="'warehouse-area-type-delete'"
...
export default {
  ...
  methods: {
    ...
    filterWarehouseAreaType() {
      this.loading = true;
      WarehouseAreaTypeService.filterWarehouseAreaType(this.currentPage, this.formSearch)
        .then((res) => {
          if (res.data.code === -2) {
...
// WEBPACK FOOTER //
// warehouse-area-type.vue?3783fefe