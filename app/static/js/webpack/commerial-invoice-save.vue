<template>
  <div class="CI-header-save">
    <div class="header">
      ...
      <label class="title">{{
        orderId ? "Edit Commerial Invoice" : "Add Commerial Invoice"
      }}</label>
    </div>
    <div class="content-wrap">
      <div class="content content-width" v-loading.body="loading">
        ...
        <div class="order-info-two">
          ...
          <div class="pi-left">
            <el-tag size="small" :key="tag.id" v-for="tag in plTags">{{
              tag.pi_no
            }}</el-tag>
          </div>
        </div>
        ...
        <div class="parts-list">
          <span class="att-total first-total"
            >Total Amount to be paid: {{ currency | currencyType
            }}{{ totalAmountCount }}</span
          >
...
export default {
  ...
  methods: {
    ...
    checkParams() {
      ...
      if (this.formData.total_amount <= 0 || this.formData.total_amount === "") {
        this.$message({
      ...
      if (this.formData.total_volume <= 0 || this.formData.total_volume === "") {
        this.$message({
...
// WEBPACK FOOTER //
// commerial-invoice-save.vue?0075ccee