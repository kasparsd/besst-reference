...
export default {
  ...
  i18n: {
    messages: {
      fr: FR.Components.ServiceCenter.accountList,
  ...
  mounted: function () {
    ...
    this.$root.eventHub.$on(Core.Const.DATA.KEY_I18N_UPDATE, () => {
      setTimeout(() => {
        vm.tableHead = [
          vm.$t("tableHead.account"),
          vm.$t("tableHead.createTime"),
          vm.$t("tableHead.operation"),
        ];
      }, 100);
    });
  ...
  methods: {
    ...
    getAccountList() {
      Core.Api.Org.getServiceCenterAccountList(this.orgId, this.currentPage).then(
        (res) => {
    ...
    addAccount() {
      ...
      Core.Api.Org.createServiceCenterAccount(this.orgId, this.email, this.password).then(
        (res) => {
