<template>
  <div class="model-bom-list">
    <div class="header">
      ...
      <div class="btn-group-top">
        <div class="icons">
          <div
            class="icon-item"
            :class="[!isEdit ? 'edit' : 'save']"
            @click="editBom"
            v-if="authList[Core.Const.AUTH_LIST.itemBomEdit]"
            :title="btnText"
          ></div>
          <div
            class="icon-item print"
            @click="printBom"
            v-show="!isEdit"
            :title="$t('print')"
          ></div>
          ...
          <div
            ...
            v-if="authList[Core.Const.AUTH_LIST.itemBomDelete]"
            :title="$t('delete')"
          ></div>
        </div>
      </div>
      ...
      <div class="content">
        <div class="part top">
          ...
          <div class="img-div">
            ...
            <el-button
              type="primary"
              @click="largeImgDialogVisible = true"
              :disabled="!upload.photographSrc"
              >{{ $t("viewLargerImage") }}
            </el-button>
          </div>
        </div>
    ...
    <el-dialog
      ...
      :visible.sync="bomListDeleteDialogVisible"
      size="tiny"
    >
      <p>{{ $t("deleteProductDialog.content") }}</p>
      <div slot="footer" class="dialog-footer">
        <el-button type="preview" @click="deleteBom">{{ $t("yes") }}</el-button>
        <el-button type="primary" @click="bomListDeleteDialogVisible = false">{{
          $t("no")
        }}</el-button>
      </div>
    </el-dialog>
    ...
    <el-dialog
      title="Larger-Image"
      :visible.sync="largeImgDialogVisible"
      size="large"
      class="large-image"
    >
...
export default {
  ...
  methods: {
    ...
    queryOrderItem() {
      let queryList = [];
      Core.Api.Item.queryComponentItem(this.selectCategoryItem.id, this.partName).then(
        (res) => {
          queryList = res.query;
    ...
    getAllCategory() {
      Core.Api.Category.getCategoryListOfAll().then((res) => {
