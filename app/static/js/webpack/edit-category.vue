  ...
  <div class="edit-category">
    ...
    <div class="content">
      ...
      <div class="category-tree">
        <el-tree :data="categoryList" :props="defaultProps" default-expand-all :render-content="renderContent"> </el-tree>
      </div>
    </div>
...
export default {
  ...
  i18n: {
    messages: {
      fr: FR.Component.Product.EditCategory,
  ...
  methods: {
    getList() {
      Core.Api.Category.getAllCategoryList(Core.Const.COMPONENT_ROOT_CATEGORY).then((res) => {
    ...
    toAttribute(data) {
      this.currentID = data.id
      this.$router.push({
        name: 'componentProductAttribute',
        params: { categoryId: this.currentID }
      })
    },
    //删除当前分组
    deleteCategory() {
        ...
        <span>
          ...
          <span style="float: right; margin-right: 20px">
            {node.level > 0 && node.level < 4 ? (
              ...
              >
                {this.$t('addSubCategory')}
              </el-button>
            ) : null}
            {node.level > 1 && node.level < 5 ? (
              <el-button
                size="mini"
                type="text"
                on-click={(e) => {
            ...
            {node.level > 1 && node.level < 4 ? (
              <el-button
                ...
                on-click={(e) => {
                  e.stopPropagation()
                  this.showDeleteDialog(store, data)
                }}
              >
                {this.$t('delete')}
              </el-button>
