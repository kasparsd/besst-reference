  ...
  <div class="account">
    ...
    <div class="main-warp">
      ...
      <div @keyup.13="sendCode">
        ...
        >
          <el-button slot="append" :disabled="isSend" @click="sendCode">{{
            text
          }}</el-button>
        </el-input>
      </div>
      ...
      <div v-show="show" class="tip" :class="{ error: isError }">
        <p>{{ message }}</p>
      </div>
      <div @keyup.13="goCompany">
        <el-input
          class="input"
          :maxlength="4"
          :placeholder="$t('emailValidationCode')"
          v-model="account.validationCode"
        ></el-input>
        <br />
        <el-input
          class="input"
      ...
      <el-button class="next" @click="goCompany" type="primary">{{
        $t("next")
      }}</el-button>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    sendCode() {
      ...
      this.isError = false;
      this.show = false;
      Core.Api.Common.sendEmailVCode(this.account.email.replace(/\s+/g, "")).then(
        (res) => {
          this.$message({
    ...
    goCompany() {
      ...
      Core.Api.User.register(
        this.account.email,
        this.account.password,
        this.account.validationCode,
        this.companyName
      ).then((res) => {
        this.$message({
