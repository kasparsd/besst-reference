...
export default {
  ...
  methods: {
      ...
      const data = [
        ...
        },
        {
          ...
          items: [
            ...
            {
              label: this.$t('cNam.torque'),
              value: this.torqueValue + 'mV'
            },
            {
              label: this.$t('cNam.cadence'),
              value: this.cadence + 'RPM'
            }
          ]
        },
        {
    ...
    writeSn() {
      ...
      Sensor.port.usbSDK.machineInformation.update
        .serialNumber(0x05, 0x01, Sensor.port.Core.Util.stringToHex(this.sn))
        .then((res) => {
          //                        console.log(res);
          if (res.can === 'NORMAL_ACK') {
            ...
            Sensor.port.usbSDK.machineInformation.get
    ...
    writeParamWithAuth() {
      ...
      if (this.clientCode) {
        if (this.clientCode.length < 25) {
          Sensor.port.usbSDK.machineInformation.update
            .customerNumber(0x05, 0x01, Sensor.port.Core.Util.stringToHex(this.clientCode))
            .then((res) => {
              //                                    console.log(res);
              if (res.can === 'NORMAL_ACK') {
                this.clientCode = ''
    ...
    clickWriteReset() {
      if (this.modelWrite && this.modelWrite.indexOf('SR') > -1) {
        Sensor.port.usbSDK.machineInformation.update
          .modelNumber(0x05, 0x01, Sensor.port.Core.Util.stringToHex(this.modelWrite.trim()))
          .then((res) => {
            console.log(res)
            if (res.can === 'NORMAL_ACK') {
              this.modelWrite = ''
      ...
      if (this.hardwareWrite && this.hardwareWrite.indexOf('SR') > -1) {
        Sensor.port.usbSDK.machineInformation.update
          .hardwareVersionNumber(0x05, 0x01, Sensor.port.Core.Util.stringToHex(this.hardwareWrite.trim()))
          .then((res) => {
            console.log(res)
            if (res.can === 'NORMAL_ACK') {
              this.hardwareWrite = ''
              ...
              Sensor.port.usbSDK.machineInformation.get
                .hardwareVersionNumber(0x05, 0x01)
                .then((res) => {
    ...
    writeManufacturer() {
      Sensor.port.usbSDK.machineInformation.update
        .manufacturer(0x05, 0x01, Sensor.port.Core.Util.stringToHex(this.manufacturer.trim()))
        .then((res) => {
          if (res.can === 'NORMAL_ACK') {
            this.$message({
