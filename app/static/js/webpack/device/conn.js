...
function getUsbBasic(target, base) {
  ...
  usbSDK.machineInformation.get
    .serialNumber(0x05, target)
    .then((res) => {
  ...
  usbSDK.machineInformation.get
    .softwareVersionNumber(0x05, target)
    .then((res) => {
  ...
  if (!base) {
    usbSDK.machineInformation.get
      .hardwareVersionNumber(0x05, target)
      .then((res) => {
...
function getBatteryOfData() {
  usbSDK.machineInformation.get.designInformation_v2(0x05, 0x04).then((data) => {
    const { parsedInfo } = data
    const setLength = parsedInfo.values[0]
    // 根据长度创建一个数组将数组填充为0
    const arr = new Array(setLength).fill(0)
    usbSDK.machineInformation.get
      .chargingInformation(0x05, 0x04)
      .then((res) => {
        let result_3 = Buffer.from(res.data, 'hex')
        if (result_3.length >= 6) {
      ...
      .finally(() => {})
    usbSDK.machineInformation.get
      .batteryCellsInformation(0x05, 0x04)
      .then((res) => {
        const result_4 = Buffer.from(res.data, 'hex')
        if (result_4.length === 8) {
          arr[0] = result_4[0] + result_4[1] * 256
          arr[1] = result_4[2] + result_4[3] * 256
          arr[2] = result_4[4] + result_4[5] * 256
          arr[3] = result_4[6] + result_4[7] * 256
        }
      })
      .catch((err) => {
        console.log(err)
      ...
      .finally(() => {})
    usbSDK.machineInformation.get
      .batteryCellsInformation_1(0x05, 0x04)
      .then((res) => {
        const result_5 = Buffer.from(res.data, 'hex')
        if (result_5.length === 8) {
          arr[4] = result_5[0] + result_5[1] * 256
          arr[5] = result_5[2] + result_5[3] * 256
          arr[6] = result_5[4] + result_5[5] * 256
          arr[7] = result_5[6] + result_5[7] * 256
          const arr1to7 = arr.slice(0, setLength < 7 ? setLength : 7)
          const cell1_7 = arr1to7.join()
      ...
      .finally(() => {})
    usbSDK.machineInformation.get
      .batteryCellsInformation_2(0x05, 0x04)
      .then((res) => {
        let result_6 = Buffer.from(res.data, 'hex')
        if (result_6.length >= 4) {
          arr[8] = result_6[0] + result_6[1] * 256
          arr[9] = result_6[2] + result_6[3] * 256
          if (result_6.length >= 6) {
            arr[10] = result_6[4] + result_6[5] * 256
            arr[11] = result_6[6] + result_6[7] * 256
          }
          let arr8to14 = arr.slice(7, setLength)
      ...
      .finally(() => {})
    usbSDK.machineInformation.get
      .batteryCellsInformation_3(0x05, 0x04)
      .then((res) => {
        let result_7 = Buffer.from(res.data, 'hex')
        if (result_7.length >= 4) {
          arr[12] = result_7[0] + result_7[1] * 256
          arr[13] = result_7[2] + result_7[3] * 256
          const arr8to14 = arr.slice(7, setLength)
          const cell8_14 = arr8to14.join()
          getUsbBatteryBus.next({ type: 'data', data: { cell8_14: cell8_14 } })
        }
      })
      .catch((err) => {
