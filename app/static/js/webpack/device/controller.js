...
function writeSn(data) {
  ...
  if (Port.getUSBDevice()) {
    ...
    setTimeout(() => {
      Port.usbSDK.machineInformation.uart.ctl
        .basicInfo()
        .then((res) => {
          // console.log(res);
        })
...
function readBasic(param) {
  ...
  if (param && param === 'b') {
    readKey = ['model', 'sn', 'version', 'hardware_version']
    retryTime = 5 * basicTime + 50
  } else {
    readKey = ['model', 'sn', 'version', 'hardware_version', 'limit_volt', 'man_name']
...
// 获取控制器数据
function getControllerData(target) {
  return Port.usbSDK.publicInformation.get.controllerInformation_3203(0x05, target)
}
