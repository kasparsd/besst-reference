<template>
  <div class="PI-header">
    <div class="header">
      ...
      <label class="title">{{
        orderId ? "Edit Proforma Invoice" : "Add Proforma Invoice"
      }}</label>
    </div>
          ...
          <table class="table-content" border="0" cellspacing="0" cellpadding="0">
            ...
            <tbody>
              <tr v-for="(item, index) in orderItems">
                ...
                <td>{{ item.amount }}</td>
                <td>
                  <a class="btn-underline" v-on:click="deleteItem(item, index)">{{
                    $t("delete")
                  }}</a>
                </td>
              </tr>
...
export default {
    ...
    if (scope.orderId) {
      ...
      scope
        ...
        .then((res) => {
          ...
          scope.formData.date = Core.Util.changeDateForm(scope.formData.date * 1000);
          scope.formData.etd = Core.Util.changeDateForm(scope.formData.etd * 1000);
  ...
  methods: {
        ...
        PurchaseService.getPurchase(_orderNo)
          .then((res) => {
            ...
            if (res.data.content.length > 0) {
              ...
              return PurchaseService.filterAllPurchaseDetails(purchase_order_id);
            }
    ...
    getAllCategory() {
      this.loading = true;
      Core.Api.Category.getAllCategoryListOfBafang(Core.Const.COMPONENT_ROOT_CATEGORY)
        .then((res) => {
          if (res.code === -2) {
            throw res.message;
          }
...
.PI-header {
  ...
  .content {
    ...
    .table-wrap {
      .el-input-group__prepend .el-select.currency-input .el-input .el-input__inner {
        width: 75px;
      }
...
// WEBPACK FOOTER //
// proforma-invoice-header.vue?0fd6e072