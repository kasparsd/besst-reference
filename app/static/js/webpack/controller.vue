<template>
  <div class="controller">
    <div class="controller-content">
      <div class="title">
        ...
        <div class="connect-tip">
          <el-button type="primary" @click.stop="onClickPrint">Preview</el-button>
      ...
      <div class="info">
        ...
        <div class="part">
          ...
          <div class="btn-row">
            <div class="btn-group">
              <el-button type="primary" class="btn-write" @click="writeSn" :disabled="loading" v-if="org.type === Core.Const.ORG_TYPE.COMPONENT"
                >{{ $t('cNam.write') }}
              </el-button>
            </div>
          </div>
          <div class="item" v-if="customer_sn">
            <label class="label">Customer SN</label>
            <el-input class="input" v-model="customer_sn"></el-input>
          </div>
          <div class="item">
        ...
        <div class="part right">
          <div class="com" v-if="!usbHID">
            ...
            <el-select v-model="com" @change="changeCom" class="com-select" @click.native="getComList" :placeholder="$t('cNam.select')">
          ...
          <div class="btn-row">
            <div class="btn-group">
              ...
              <el-button class="btn-write" type="preview" @click="readBase" :disabled="loading || isReset || !isConnected"
                >{{ $t('cNam.read') }}
              </el-button>
      ...
      <div class="info" v-if="isCan">
        ...
        <div class="part">
          <div class="item">
            <label class="label">{{ $t('cNam.speedLimit') }}</label>
            <el-select v-model="speedLimit" class="com-select" filterable :placeholder="$t('cNam.select')">
              <el-option v-for="(item, index) in speedLimits" :key="index" :label="item" :value="item"> </el-option>
            </el-select>
          </div>
          <div class="item">
            <label class="label">{{ $t('cNam.wheelDiameter') }}</label>
            <el-select v-model="wheelDiameter" @change="updateWheelCircumference" class="com-select" filterable :placeholder="$t('cNam.select')">
              <el-option v-for="(item, index) in wheelDiameters" :key="index" :label="item.label" :value="item.value"> </el-option>
            </el-select>
          </div>
          <div class="item">
            <label class="label">{{ $t('cNam.circumference') }}</label>
            <el-select v-model="circumference" class="com-select" filterable allow-create :placeholder="$t('cNam.select')">
              <el-option v-for="(item, index) in circumferences" :key="index" :label="item" :value="item"> </el-option>
            </el-select>
          </div>
          <div class="btn-row">
            <div class="btn-group">
              <el-button type="primary" :disabled="circumference == '' || wheelDiameter == ''" class="btn-write" @click="clickWriteParam()">{{
                $t('cNam.write')
              }}</el-button>
            </div>
          </div>
        ...
        <div class="part right">
          ...
          <div class="item">
            ...
            <el-input class="input" v-model="paramsCode" placeholder="6006"></el-input>
          </div>
          <div class="btn-row">
            <div class="btn-group">
              <el-button type="preview" @click="readParamsCode">{{ $t('cNam.read') }}</el-button>
              <el-button type="primary" @click="writeParamsCode">{{ $t('cNam.write') }}</el-button>
            </div>
          </div>
          <div class="item">
          ...
          <div class="item">
            ...
            <span style="padding: 0 20px 0 20px" v-if="sensorStart">{{ $t('cNam.start') }}</span>
          </div>
          <div class="item">
            <label class="label">{{ $t('cNam.torqueSenCali') }}</label>
            <el-button class="btn-connect" @click="sensorZeroSetStart">{{ $t('cNam.start') }}</el-button>
            <span style="padding: 0 20px 0 20px" v-if="sensorZeroStart">{{ $t('cNam.start') }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="info" v-show="isCan && (org.type === Core.Const.ORG_TYPE.COMPONENT || org.id === 1340 || org.id === 1341 || org.id === 1212)">
        ...
        <div>
          <div>
            <div class="con-Params">
              Params Read:
              <el-input type="textarea" :rows="2" resize="none" placeholder="6011" v-model="conParamsRead_11"></el-input>
              <el-button class="btn-connect" type="preview" @click="conParamsRead_6011">{{ $t('cNam.read') }}6011</el-button>
            </div>
            <div class="con-Params">
              Params Write:
              <el-input type="textarea" :rows="2" resize="none" placeholder="6011" v-model="conParamsWrite_11"></el-input>
          ...
          <div>
            <div class="con-Params">
              Params Read:
              <el-input type="textarea" :rows="2" resize="none" placeholder="6012" v-model="conParamsRead_12"></el-input>
              <el-button class="btn-connect" type="preview" @click="conParamsRead_6012">{{ $t('cNam.read') }}6012</el-button>
            </div>
            <div class="con-Params">
              Params Write:
              <el-input type="textarea" :rows="2" resize="none" placeholder="6012" v-model="conParamsWrite_12"></el-input>
          ...
          <template>
            <el-collapse v-model="openCollapse">
              <el-collapse-item v-for="(data, index) in dataObject" :key="index" :name="index" :title="Object.keys(data)[0]">
                <div>
                  <div class="table-wrap">
                    <table class="table-content" border="0" cellspacing="0" cellpadding="0">
                      <thead>
                        <td v-for="(item, tidx) in tableHead" :key="tidx">{{ item }}</td>
                      </thead>
                      <tbody class="height2">
                        <tr v-for="(item, didx) in data[Object.keys(data)[0]]" :key="didx">
                          <td class="long-td">
                          ...
                          <td class="long-td" v-if="Array.isArray(item.range) && typeof item.range[0] == 'number'">
                            <el-select v-model="item.set_value" :placeholder="$t('cNam.select')" class="input">
                              <el-option v-for="(i, index) in item.range" :key="index" :label="i" :value="i"> </el-option>
                            </el-select>
                          </td>
                          <td class="long-td" v-else-if="Array.isArray(item.range)">
                            <el-select v-model="item.set_value" :placeholder="$t('cNam.select')" class="input">
                              <el-option v-for="ii in item.range" :key="ii.id" :label="ii.value" :value="ii.id"> </el-option>
                            </el-select>
                          </td>
          ...
          <div v-show="false">
            <div class="con-Params">
              Params Read:
              <el-input type="textarea" :rows="2" resize="none" placeholder="6017" v-model="conParamsRead_17"></el-input>
              <el-button class="btn-connect" type="preview" @click="conParamsRead_6017">{{ $t('cNam.read') }}6017</el-button>
            </div>
            <div class="con-Params">
              Params Write:
              <el-input type="textarea" :rows="2" resize="none" placeholder="6017" v-model="conParamsWrite_17"></el-input>
          ...
          <div v-show="false">
            <div class="con-Params">
              Params Read:
              <el-input type="textarea" :rows="2" resize="none" placeholder="6018" v-model="conParamsRead_18"></el-input>
              <el-button class="btn-connect" type="preview" @click="conParamsRead_6018">{{ $t('cNam.read') }}6018</el-button>
            </div>
            <div class="con-Params">
              Params Write:
              <el-input type="textarea" :rows="2" resize="none" placeholder="6018" v-model="conParamsWrite_18"></el-input>
    ...
    <!--说明弹框-->
    <el-dialog :title="$t('cNam.operationTip')" custom-class="tip-dialog" :visible.sync="dialogTipVisible">
      <div>
        <h1>{{ $t('cNam.txtTitle1') }}</h1>
        <div class="txt">
          <span class="itemName">COM</span> <span>{{ $t('cNam.txtTitleCom') }}</span>
        </div>
        <div class="txt">
          <span class="itemName">USB HID</span> <span>{{ $t('cNam.txtTitleHid') }}</span>
        </div>
        <div class="txt">
          <span class="itemName">{{ $t('cNam.connect') }}</span>
          <span>{{ $t('cNam.txtTitleCon') }}</span>
        </div>
        <div class="txt">
          <span class="itemName">{{ $t('cNam.read') }}</span>
          <span>{{ $t('cNam.txtTitleRead') }}</span>
        </div>
        <div class="txt" v-if="org.type === Core.Const.ORG_TYPE.COMPONENT">
          <span class="itemName">{{ $t('cNam.write') }}</span>
          <span>{{ $t('cNam.txtTitleWrt') }}</span>
        </div>
        <div class="txt">
          <span class="itemName">{{ $t('cNam.reset') }}</span>
          <span>{{ $t('cNam.txtTitleReset') }}</span>
        </div>
    ...
    <el-dialog :title="$t('auth')" :visible.sync="dialogWriteAuthVisible">
      <div>
        <label>
...
import CustomerSNService from 'src/services/sn/CustomerSNService.js'
...
export default {
  ...
  i18n: {
    messages: {
      fr: FR.Components.Tool.Controller,
  ...
  mounted() {
    ...
    if (Core.Data.getOrg()) {
      this.isDealerOrAssembler =
        Core.Data.getOrg().type === Core.Const.ORG_TYPE.DEALER ||
        Core.Data.getOrg().type === Core.Const.ORG_TYPE.OEM ||
        Core.Data.getOrg().type === Core.Const.ORG_TYPE.SPECIAL_DEAlER
    }
  ...
  methods: {
    onClickPrint() {
      const data = [
        {
          ...
          items: [
            ...
            {
              label: this.$t('cNam.com'),
              value: this.com
            },
            {
              label: 'Customer SN',
              value: this.customer_sn
            },
            {
              label: this.$t('cNam.model'),
              value: this.model
            },
            {
              label: this.$t('cNam.softwareVersion'),
              value: this.version
            },
            {
              label: this.$t('cNam.hardwareVersion'),
              value: this.hardVersion
            }
          ]
        }
      ]
      ...
      const UartPara = [
        {
          title: this.$t('cNam.parameter'),
          items: [
            {
              label: this.$t('cNam.spdMeterType'),
              value: this.spdMeterType
            },
            {
              label: this.$t('cNam.nmlVolt'),
              value: this.nmlVolt + 'V'
            },

            {
              label: this.$t('cNam.manName'),
              value: this.manNameRead
            },

            {
              label: this.$t('cNam.spdMeterSignalNum'),
              value: this.spdMeterSignalNum
            },
            {
              label: this.$t('cNam.maxCur'),
              value: this.maxCur + 'A'
            },
            // Uart 页面逻辑
            {
              label: this.$t('cNam.limitVolt'),
              value: this.limitVolt
            },
            // 限速限流
            {
              label: this.$t('cNam.assit') + '0',
              value: `${this.curList[0] || ' - '} ${this.$t('cNam.limitCurrent')}(%) ${this.spdList[0] || ' - '} ${this.$t('cNam.limitSpd')}(%)`
            },
            {
              label: this.$t('cNam.assit') + '1',
              value: `${this.curList[1] || ' - '} ${this.$t('cNam.limitCurrent')}(%) ${this.spdList[1] || ' - '} ${this.$t('cNam.limitSpd')}(%)`
            },
            {
              label: this.$t('cNam.assit') + '2',
              value: `${this.curList[2] || ' - '} ${this.$t('cNam.limitCurrent')}(%) ${this.spdList[2] || ' - '} ${this.$t('cNam.limitSpd')}(%)`
            },
            {
              label: this.$t('cNam.assit') + '3',
              value: `${this.curList[3] || ' - '} ${this.$t('cNam.limitCurrent')}(%) ${this.spdList[3] || ' - '} ${this.$t('cNam.limitSpd')}(%)`
            },
            {
              label: this.$t('cNam.assit') + '4',
              value: `${this.curList[4] || ' - '} ${this.$t('cNam.limitCurrent')}(%) ${this.spdList[4] || ' - '} ${this.$t('cNam.limitSpd')}(%)`
            },
            {
              label: this.$t('cNam.limitCurrent'),
              value: this.limitCur
            },
            {
              label: this.$t('cNam.assit') + '5',
              value: `${this.curList[5] || ' - '} ${this.$t('cNam.limitCurrent')}(%) ${this.spdList[5] || ' - '} ${this.$t('cNam.limitSpd')}(%)`
            },
            {
              label: this.$t('cNam.assit') + '6',
              value: `${this.curList[6] || ' - '} ${this.$t('cNam.limitCurrent')}(%) ${this.spdList[6] || ' - '} ${this.$t('cNam.limitSpd')}(%)`
            },
            {
              label: this.$t('cNam.assit') + '7',
              value: `${this.curList[7] || ' - '} ${this.$t('cNam.limitCurrent')}(%) ${this.spdList[7] || ' - '} ${this.$t('cNam.limitSpd')}(%)`
            },
            {
              label: this.$t('cNam.assit') + '8',
              value: `${this.curList[8] || ' - '} ${this.$t('cNam.limitCurrent')}(%) ${this.spdList[8] || ' - '} ${this.$t('cNam.limitSpd')}(%)`
            },
            {
              label: this.$t('cNam.assit') + '9',
              value: `${this.curList[9] || ' - '} ${this.$t('cNam.limitCurrent')}(%) ${this.spdList[9] || ' - '} ${this.$t('cNam.limitSpd')}(%)`
            }
          ]
        }
      ]
      ...
      const CanPara = [
        {
          ...
          items: [
            {
              label: this.$t('cNam.clientCode'),
              value: this.clientCodeRead
            },

            {
              label: this.$t('cNam.speedLimit'),
              value: this.speedLimitRead + 'km/h'
            },
            {
              label: this.$t('cNam.wheelDiameter'),
              value: this.wheelDiameterRead
            },
            {
              label: this.$t('cNam.circumference'),
              value: this.circumferenceRead + 'mm'
            },
            {
              label: this.$t('cNam.conSpeed'),
              value: this.conSpeed + 'Km/h'
            },
            {
              label: this.$t('cNam.conCurrent'),
              value: this.conCurrent + 'A'
            },
            {
              label: this.$t('cNam.conVoltage'),
              value: this.conVoltage + 'V'
            },
            {
              label: this.$t('cNam.motorTemperature'),
              value: this.motorTemperature + '℃'
            },
            {
              label: this.$t('cNam.capacityLeft'),
              value: this.conCapacityLeft + '%'
            },
            {
              label: this.$t('cNam.conSingleTrip'),
              value: this.conSingleTrip + 'Km'
            },
            {
              label: this.$t('cNam.conSingleTripLeft'),
              value: this.conSingleTripLeft + 'Km'
            },
            {
              label: this.$t('cNam.conCadence'),
              value: this.conCadence + 'RPM'
            },
            {
              label: this.$t('cNam.conTorque'),
              value: this.conTorque + 'mV'
            },
            {
              label: this.$t('cNam.calorie'),
              value: this.calorie + 'Kcal'
            },
            {
              label: this.$t('cNam.assistStatus'),
              value: this.assistance
            },
            {
              label: this.$t('cNam.conTemperature'),
              value: this.conTemperature + '℃'
            }
          ]
        }
      ]

      this.isCan ? data.push(...CanPara) : data.push(...UartPara)

      const CanSet = [
        {
          title: this.$t('cNam.setting'),
          items: [
            {
              label: this.$t('cNam.speedLimit'),
            ...
            {
              label: this.$t('cNam.wheelDiameter'),
              value: this.wheelDiameter
            },
            {
              label: this.$t('cNam.circumference'),
              value: this.circumference
            },
            {
              label: this.$t('cNam.clientCode'),
            ...
            {
              label: this.$t('cNam.errorCode'),
              value: this.errorCode
            },
            {
              label: this.$t('cNam.paramsName'),
              value: this.paramsName
            },
            {
              label: this.$t('cNam.bootLoad'),
              value: this.bootLoad
            }
          ]
        }
      ...
      this.isCan ? data.push(...CanSet) : ''

      console.log(data)
      this.$refs['printPreview'].show(data)
    },
    initCtrlParamsObject() {},
    ...
    conParamsRead_6011() {
      //                this.conParamsRead_11 = "24010203FF04FF05000600070809101112131400151600011718011900202101220023002400250011121314151617181900004344450001464748495000ffe5";
      //                this.conParamsParse(this.conParamsRead_11, this.ctrlParams6011);

      // 从控制器读取
      Controller.port.usbSDK.machineInformation.get
        .conParams_1(0x05, 0x02)
        .then((res) => {
    ...
    conParamsRead_6012() {
      //                this.conParamsRead_12 = "0f0c090807071e1e201c190c060605050404646464646432231e19140f0a06060505040403030302020232281e14100c50785046463c03ffffffffffffffff52";
      //                this.conParamsParse(this.conParamsRead_12, this.ctrlParams6012);

      //                // 从控制器读取
      Controller.port.usbSDK.machineInformation.get
        .conParams_2(0x05, 0x02)
        .then((res) => {
          //                        getUsbBasicBus.next({type: 'data', data: {'model':res.dataParsed}});
    ...
    conParamsCombine(type, ctrlParams) {
      ...
      ctrlParams.forEach((item, index) => {
        ...
        if (index === 0 && dataIndex !== 0) {
          ...
          data =
            'ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff'.substr(
              0,
              firstIndex * 2
            ) + data
        }
        ...
        if (dataIndex - dataIndexLast > 1) {
          dataSupply =
            'FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF'.substr(
              0,
              2 * (dataIndex - dataIndexLast - 1)
            )
        }
      ...
      //                console.log(type, checkSum, value);
    ...
    conParamsUpdate_6011() {
      ...
      Controller.port.usbSDK.machineInformation.update
        .conParams_1(0x05, 0x02, this.conParamsWrite_11)
        .then((res) => {
          //                        console.log(res);
    ...
    conParamsUpdate_6012() {
      ...
      Controller.port.usbSDK.machineInformation.update
        .conParams_2(0x05, 0x02, this.conParamsWrite_12)
        .then((res) => {
          //                        console.log(res);
          if (res.can === 'NORMAL_ACK') {
            this.$message({
    ...
    conParamsUpdate_6014() {
      ...
      Controller.port.usbSDK.machineInformation.update
        .conParams_4(0x05, 0x02, this.conParamsWrite_14)
        .then((res) => {
          console.log(res)
          if (res.can == 'NORMAL_ACK') {
            this.$message({
    ...
    conParamsUpdate_6017() {
      ...
      Controller.port.usbSDK.machineInformation.update
        .conParams_7(0x05, 0x02, this.conParamsWrite_17)
        .then((res) => {
          //                        console.log(res);
          if (res.can === 'NORMAL_ACK') {
            this.$message({
    ...
    conParamsUpdate_6018() {
      ...
      Controller.port.usbSDK.machineInformation.update
        .conParams_8(0x05, 0x02, this.conParamsWrite_18)
        .then((res) => {
          console.log(res)
          if (res.can == 'NORMAL_ACK') {
            this.$message({
      ...
      if (!this.isCan || Controller.port.checkUsbUart()) {
        ...
        Controller.getBus().subscribe((res) => {
          ...
          if (res.data.ctrl.hasOwnProperty('sn')) {
            ...
            if (this.sn !== '') {
              // 查询当前sn是否存在客户sn
              CustomerSNService.findCustomerSN(this.sn).then(({ data }) => {
                const { customerSn } = data.data
                this.customer_sn = customerSn || ''
              })
            }
    ...
    writeSnWithAuth() {
      ...
      if (!this.isCan || Controller.port.checkUsbUart()) {
        ...
        Controller.port.usbSDK.machineInformation.update
          .serialNumber(0x05, 0x02, Controller.port.Core.Util.stringToHex(this.sn))
          .then((res) => {
            //                            console.log(res);
            if (res.can === 'NORMAL_ACK') {
              this.sn = ''
    ...
    clickWriteReset() {
      if (this.modelWrite && this.modelWrite.indexOf('CR') > -1) {
        Controller.port.usbSDK.machineInformation.update
          .modelNumber(0x05, 0x02, Controller.port.Core.Util.stringToHex(this.modelWrite.trim()))
          .then((res) => {
            //                            console.log(res);
            if (res.can === 'NORMAL_ACK') {
              this.modelWrite = ''
      ...
      if (this.hardwareWrite && this.hardwareWrite.indexOf('CR') > -1) {
        Controller.port.usbSDK.machineInformation.update
          .hardwareVersionNumber(0x05, 0x02, Controller.port.Core.Util.stringToHex(this.hardwareWrite.trim()))
          .then((res) => {
            //                            console.log(res);
            if (res.can === 'NORMAL_ACK') {
              this.hardwareWrite = ''
              ...
              Controller.port.usbSDK.machineInformation.get
                .hardwareVersionNumber(0x05, 0x02)
                .then((res) => {
    ...
    clickWriteParam() {
      ...
      // 经销商和组装厂不能写比当前高的数据
      if (
        this.org.type === Core.Const.ORG_TYPE.DEALER ||
        this.org.type === Core.Const.ORG_TYPE.OEM ||
        this.org.type === Core.Const.ORG_TYPE.SPECIAL_DEAlER
      ) {
    ...
    writeSpeed(data) {
      ...
      let spdStr = Buffer.from([(spd >> 8) & 0xff, spd & 0xff])
        .reverse()
        .toString('hex')
      let cirBuf = Buffer.from([(cir >> 8) & 0xff, cir & 0xff])
        .reverse()
        .toString('hex')
      //                Port.writeByQueueAndWait(makeFrameCanWrite('00','3203', spdStr + wheel +cirBuf), 100);
      //                console.log(spdStr + this.wheelDiameter + cirBuf);
      Controller.port.usbSDK.publicInformation.update
        .controllerInformation_3(0x05, 0x02, spdStr + this.wheelDiameter + cirBuf)
        .then((res) => {
          //                        console.log(res);
    ...
    writeClientCode() {
      ...
      if (this.clientCode.length < 25) {
        Controller.port.usbSDK.machineInformation.update
          .customerNumber(0x05, 0x02, Controller.port.Core.Util.stringToHex(this.clientCode))
          .then((res) => {
            //                            console.log(res);
            if (res.can === 'NORMAL_ACK') {
              this.clientCode = ''
    ...
    writeManufacturer() {
      Controller.port.usbSDK.machineInformation.update
        .manufacturer(0x05, 0x02, Controller.port.Core.Util.stringToHex(this.manName.trim()))
        .then((res) => {
          if (res.can === 'NORMAL_ACK') {
            this.$message({
    ...
    readParamsCode() {
      ...
      Controller.port.usbSDK.machineInformation.get
        .params(0x05, 0x02)
        .then((res) => {
          this.paramsCode = res.dataParsed.replace(/\s+/g, '')
          this.$message({
            message: this.$t('readSuccess'),
            type: 'success'
          })
    ...
    writeParamsCode() {
      Controller.port.usbSDK.machineInformation.update
        .params(0x05, 0x02, Controller.port.Core.Util.stringToHex(this.paramsCode.trim()))
        .then((res) => {
          if (res.can === 'NORMAL_ACK') {
            this.$message({
              message: this.$t('writeSuccess'),
    ...
    sensorSetStart() {
      ...
      Controller.port.getListenData().subscribe((res) => {
        ...
        if (res.data.code === '12' && res.data.subCode === '00') {
          ...
          if (res.data.data === '08') {
            this.sensorStartState = 1
            //                            this.$message({
            //                                message: "正在标定",
            //                                type: 'info'
            //                            });
          } else if (res.data.data === '00') {
            if (this.sensorStart && this.sensorStartState === 1) {
              vm.$message({
                message: vm.$t('cNam.success'),
                type: 'success'
              })
        ...
        if (res.data.code === '62' && res.data.subCode === '00') {
          //                        console.log(res.data.data)
          ...
          //                                type: 'info'
          //                            });
          if (this.sensorStart && this.sensorStartState === 2) {
            let data = res.data.data.substr(0, 2)
            if (data === '00') {
              vm.$message({
                message: vm.$t('cNam.operationFail'),
                type: 'error'
              })
              this.sensorStart = false
            } else if (data === '01') {
    ...
    sensorZeroSetStart() {
      ...
      let vm = this
      Controller.port.usbSDK.machineInformation.update
        .zeroPointSensorCalibration(0x05, 0x02, '')
        .then((res) => {
