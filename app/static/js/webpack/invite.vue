  ...
  <div class="dealer-invite">
        ...
        <table border="0" cellspacing="0" cellpadding="0" class="table-content">
          ...
          <tbody>
            <tr v-for="item in inviteList">
              ...
              <td>
                {{ item.target_org_type === 2 ? $t("dealerType1") : $t("dealerType2") }}
              </td>
              ...
              <td v-if="selectStatus == 1" class="icon-td">
                ...
                <div class="icons">
                  <div
                    ...
                    v-on:click.stop="changeType(item)"
                    title="change"
                  ></div>
                </div>
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="preview" @click="createInvite" :disabled="orgType == 0">{{
          $t("confirm")
        }}</el-button>
    ...
    >
      <div>
        <div class="pt-30">
          ...
          <div>
            <el-button @click="getAllList()" :disabled="orgType === 0"
              >Get List</el-button
            >
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    getInitInvitationCode() {
      Core.Api.Org.selectInvitationCode(this.user.id)
        .then((res) => {
          //                    console.log(res);
          if (res.code) {
    ...
    // 获取邀请列表
    getInviteList() {
      Core.Api.Brand.getOrgDealerInvitationList(
        this.selectStatus,
        this.queryContent,
        this.currentPage
      ).then((res) => {
    ...
    getAllList() {
      Core.Api.Brand.getOrgInvitationAllList(this.orgType, this.selectStatus).then(
        (res) => {
    ...
    changeType(item) {
      let type = 2;
      if (item.target_org_type === 2) {
        type = 6;
      } else if (item.target_org_type === 6) {
        type = 2;
      }
      //                Core.Api.Brand.switchInvitationType(item.id, type).then(
      //                    res => {
      //                        this.$message({
    ...
    getInvitationCodeByUserId() {
      if (this.getInvitationCode == "") {
        Core.Api.Org.getOrgInvitationCode(this.user.id)
          .then((res) => {
            if (res.code) {
              this.getInvitationCode = res.code;
              this.$message({
        ...
        Core.Api.Org.getCodeNumber(this.getInvitationCode)
          .then((res) => {
    ...
    deleteInvitationCodeByUserId() {
      Core.Api.Org.getCodeNumber(this.getInvitationCode)
        .then((res) => {
          if (res.num != undefined || res.num != null) {
            if (res.num <= 0) {
              Core.Api.Org.deleteOrgInvitationCode(this.user.id)
                .then((res) => {
                  if (res == "") {
