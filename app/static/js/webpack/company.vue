<template>
  <div class="company">
    ...
    <div class="nav-btn">
      <el-button type="primary" icon="arrow-left" @click="goLogin">{{
        $t("login")
      }}</el-button>
    </div>
    <div class="main-warp">
    ...
    >
      <div class="tip-item">
        <span class="tip">
          <label>Tip:</label> <br />
          <label
            >Please confirm the country you selected which would not changed after saved.
            If your country is not exist in the option list, please contact
            besst@bafang-e.com.
          </label>
          <br />
        </span>
...
export default {
  ...
  mounted: function () {
    ...
    if (
      type === Core.Const.ORG_TYPE.DEALER ||
      type === Core.Const.ORG_TYPE.SPECIAL_DEAlER
    ) {
  ...
  methods: {
    ...
    imageUploaded(res) {
      ...
      this.upload.photographSrc =
        res.data.file && res.data.file.name ? res.data.file.name : "";
    },
      ...
      .photo-main {
        ...
        .upload-icon {
          ...
          .upload {
            width: 44px;
            height: 44px;
            background: url("../../assets/images/register/btn-upload.png") center
              no-repeat;
            background-size: cover;
          }
          .illustration {
