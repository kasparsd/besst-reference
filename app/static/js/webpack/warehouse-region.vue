  ...
  <div class="warehouse-region">
    ...
    <div class="table-wrap" v-loading.body="loading">
      <div class="table-content-holder">
        <div class="no-data-content" v-if="tableList.length <= 0 && !loading">
          noResult
        </div>
  ...
  methods: {
      ...
      this.deleteRegion(scope.currentID).then(
        (res) => {
          if (res.code === 201) {
            this.$message({
              message:
                "You can't delete the Region because it contains the warehouse that is being used.",
              type: "warning",
            });
          } else {
    ...
    required() {
      ...
      for (const item in this.regionData) {
        if (item === "remark" || item === "update_user_id" || item === "update_time") {
          continue;
        }
        if (
