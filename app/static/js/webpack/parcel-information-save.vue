<template>
  <div class="parcel-information-add">
    <div class="header">
      ...
      <label class="title">{{
        orderId ? "Edit Parcel Information" : "Add Parcel Information"
      }}</label>
    </div>
    <div class="content-wrap" v-loading.body="loading">
      <div class="content">
        ...
        <div class="footer">
          <el-button type="primary" @click="createInfo" :disabled="formData.status >= 1"
            >Save</el-button
          >
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    filterParcelInfo() {
      const scope = this;
      scope.loading = true;
      ParcelInformationService.filterParcelInfo(this.currentPage, this.formSearch)
        .then((res) => {
          if (res.data.code === -2) {
            throw res.data.message;
          }
    ...
    handleItemChange(id) {
      ...
      this.formData.country = (data && data.country) || (_coCode && _coCode.area_en);
    },
    // 添加
    dialogAdd() {
...
// WEBPACK FOOTER //
// parcel-information-save.vue?44494e90