<template>
  <div class="create-project">
      ...
      <el-collapse-item :title="item.name" :name="item.name" v-for="(item, index) in elCollapseData" :key="index">
        ...
        <el-form>
          <el-form-item label="name:" v-if="item.type != 'param'">
            <el-select v-model="item.phraseName" placeholder="请选择" @change="getSelectPhraseName">
              <el-option v-for="(item, index) in selectablePhraseNameList" :key="index" :label="item.name" :value="item.value"> </el-option>
            </el-select>
          </el-form-item>
    ...
    <el-dialog
      ...
      top="10%"
    >
      <el-select v-if="isCreate" v-model="selectedParam" placeholder="请选择" @change="getSelectedParam">
        <el-option v-for="(item, index) in selectableParamList" :key="index" :label="item.name" :value="item.value"> </el-option>
      </el-select>
    ...
    >
      <el-form>
        ...
        <el-form-item label="type:">
          <el-select v-model="selectedType" placeholder="请选择" @change="getSelectedType">
            <el-option v-for="(item, index) in categoryTypeData" :key="index" :label="item.name" :value="item.value"> </el-option>
          </el-select>
        </el-form-item>
                  ...
                  that.elCollapseData.forEach((item) => {
                    if (item.type == that.collapseType) {
                      ...
                      nextItem.selectableParamList =
                        nextItem.selectableParamList && nextItem.selectableParamList.length > 0 ? nextItem.selectableParamList : []
                    ...
                    res.data[i].forEach((childItem, index) => {
                      if (childItem != null) {
                        ...
                        if (that[info] != undefined) {
                          let propertyId =
                              childItem['property_id'] != null
                                ? childItem['property_id'].indexOf(',') > -1
                                  ? childItem['property_id'].split(',')
                                  : childItem['property_id'].split()
                                : [],
                            typeName =
                              childItem['type_name'] != null
                                ? childItem['type_name'].indexOf(',') > -1
                                  ? childItem['type_name'].split(',')
                                  : childItem['type_name'].split()
                                : [],
                            propertyValue =
                              childItem['property_value'] != null
                                ? childItem['property_value'].indexOf(',') > -1
                                  ? childItem['property_value'].split(',')
                                  : childItem['property_value'].split()
                                : []
