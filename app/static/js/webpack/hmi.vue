<template>
  <div class="hmi">
    <div class="hmi-content">
      <div class="title">
        <div style="display: flex; align-items: center"><img class="icon" src="../../assets/images/icon/icon-hmi.png" />{{ $t('hmi') }}</div>
          ...
          <div class="btn-row">
            <div class="btn-group">
              ...
              <el-button
                ...
                v-if="org.type === Core.Const.ORG_TYPE.COMPONENT"
                >{{ $t('cNam.write') }}
              </el-button>
          ...
          <div class="btn-row">
            <div class="btn-group">
              ...
              <div class="btns">
                ...
                <el-button type="primary" @click="reset" :disabled="loading || isReset || !isConnected" v-show="isCan">{{
                  $t('cNam.reset')
                }}</el-button>
      ...
      <div class="info" v-if="!isCan">
        ...
        <div class="info-left">
          ...
          <div class="item">
            ...
            <el-select v-model="wheelDiameter">
              <el-option v-for="(item, index) in wheelDiameters" :key="index" :label="item" :value="item"> </el-option>
            </el-select>
          </div>
          <div class="item">
            <label class="label">{{ $t('cNam.speedLimit') }}</label>
          ...
          <div class="btn-row">
            <div class="btn-group">
              <el-button type="primary" @click="clearServiceMileage">{{ $t('cNam.clear') }} </el-button>
            </div>
          </div>
          ...
          <div v-if="org.type === Core.Const.ORG_TYPE.COMPONENT">
            <div class="item" v-if="org.type === Core.Const.ORG_TYPE.COMPONENT">
              ...
              <el-select v-model="utcTime" :placeholder="$t('cNam.select')" class="input">
                <el-option v-for="item in timezone" :key="item.value" :label="item.label" :value="item.value"> </el-option>
              </el-select>
            </div>
            <div class="btn-row">
              <div class="btn-group">
        ...
        <div class="info-right">
          ...
          <div class="btn-row">
            <div class="btn-group">
              <el-button type="primary" @click="clearMileageUartOperation('total')">{{ $t('cNam.set') }} </el-button>
            </div>
          </div>
          <div class="item">
          ...
          <div class="btn-row">
            <div class="btn-group">
              <el-button type="primary" @click="clearMileageUartOperation('single')">{{ $t('cNam.set') }} </el-button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="info" v-else>
        ...
        <div class="info-left">
          ...
          <div class="item">
            <div class="btn-row">
              ...
              <el-button type="primary" @click="keyCardStart6501">{{ $t('cNam.start') }}</el-button>
            </div>
            <!--<div>-->
            <!--KeyData:{{keyData}}-->
            <!--</div>-->
          ...
          <div class="item">
            ...
            <el-time-picker v-model="setTime" value-format="HH:mm:ss" @change="handleChange()"> </el-time-picker>
          </div>
          <div>
            <div class="btn-row">
              <div class="btn-group">
        ...
        <div class="info-right">
          ...
          <div class="btn-row">
            <div class="btn-group">
              <el-button type="primary" @click="clearServiceMileageCan">{{ $t('cNam.clear') }} </el-button>
            </div>
          </div>
          <div class="item">
            <label class="label">{{ $t('cNam.setTotalMileage') }}</label>
          ...
          <div class="btn-row">
            <div class="btn-group">
              <el-button type="primary" @click="setTotalMileageCan">{{ $t('cNam.set') }} </el-button>
            </div>
          </div>
          <div class="item">
            <label class="label">{{ $t('cNam.setSingleMileage') }}</label>
          ...
          <div class="btn-row">
            <div class="btn-group">
              <el-button type="primary" @click="setSingleMileageCan">{{ $t('cNam.set') }} </el-button>
            </div>
          </div>
          <div class="item" v-if="org.type === Core.Const.ORG_TYPE.COMPONENT">
            <label class="label">{{ $t('cNam.spLevel') }}</label>
            <el-input class="input" v-model="spLevel" placeholder="6308"></el-input>
          </div>
          <div class="btn-row" v-if="org.type === Core.Const.ORG_TYPE.COMPONENT">
            <div class="btn-group">
              <el-button type="preview" @click="readSpLevel">{{ $t('cNam.read') }}</el-button>
            </div>
          </div>
        </div>
      </div>
    ...
    <el-dialog :title="$t('cNam.operationTip')" custom-class="tip-dialog" :visible.sync="dialogTipVisible">
      <div>
        ...
        <div class="txt">
          <span class="itemName" v-show="!isCan">{{ $t('cNam.test') }}</span>
          <span>{{ $t('cNam.txtTitleTest') }}</span>
        </div>
    ...
    <el-dialog title="Confirm PC mode" :visible.sync="pcDialogVisible" class="tip-dialog">
      <div>
        <h1>
          <label>{{ $t('cNam.note') }}:</label>{{ $t('cNam.hmiUART') }}
        </h1>
        <p></p>
        <div class="">
          <div class="txt">
...
export default {
  ...
  data() {
    return {
      ...
      showUpdateTag: false,
      checkInterval: 0 // tem test
    }
  },
  i18n: {
    messages: {
      fr: FR.Components.Tool.Hmi,
  ...
  mounted() {
    ...
    for (let i = 6; i < 35; i++) {
      ...
      if (i == 27) this.wheelDiameters.push(27.5)
    }
  ...
  methods: {
    onClickPrint() {
      const data = [
        ...
        },
        {
          ...
          items: [
            ...
            {
              label: this.$t('cNam.totalMileage'),
              value: this.totalMileage + 'km'
            },
            {
              label: this.$t('cNam.singleMileage'),
              value: this.singleMileage + 'km'
            },
            {
              label: this.$t('cNam.hmiSingleMileageClear'),
              value: this.hmiSingleTrip
            },
            {
              label: this.$t('cNam.speedMax'),
              value: this.speedMax + 'km/h'
            },
            {
              label: this.$t('cNam.speedAvg'),
              value: this.speedAverage + 'km/h'
            },
            {
              label: this.$t('cNam.hmiTurnOffTime'),
            ...
            {
              label: this.$t('cNam.totalGear'),
              value: this.totalGear
            },
            {
              label: this.$t('cNam.hmiLevel'),
              value: this.hmiLevel
            },
            {
              label: this.$t('cNam.hmiMode'),
              value: this.hmiMode
            },
            {
              label: this.$t('cNam.hmiBoost'),
              value: this.hmiBoost
            },
            {
              label: this.$t('cNam.hmiLight'),
              value: this.hmiLight
            },
            {
              label: this.$t('cNam.hmiButton'),
              value: this.hmiButton
            }
          ]
        }
      ]
      const UartSet = [
        {
          ...
          items: [
            {
              label: this.$t('cNam.serviceMileage'),
              value: this.serviceMileage
            },
            {
              label: this.$t('cNam.setTimeZone'),
              value: this.utcTime
            },
            {
              label: this.$t('cNam.setTotalMileage'),
              value: this.setTotalMileage
            },
            {
              label: this.$t('cNam.setSingleMileage'),
              value: this.setSingleMileage
            }
          ]
        }
      ]
      const CanSet = [
        {
          ...
          items: [
            ...
            {
              label: this.$t('cNam.time'),
              value: this.setTime && moment(this.setTime).format('HH:mm:ss')
            },
            {
              label: this.$t('cNam.serviceMileage'),
              value: this.serviceMileage + 'km'
            },
            {
              label: this.$t('cNam.setTotalMileage'),
              value: this.setTotalMileage + 'km'
            },
            {
              label: this.$t('cNam.setSingleMileage'),
              value: this.setSingleMileage + 'km'
            },
            {
              label: this.$t('cNam.spLevel'),
              value: this.spLevel
            }
          ]
        }
      ...
      this.isCan ? data.push(...CanSet) : data.push(...UartSet)
      console.log(data)
    ...
    readBaseInfo() {
      ...
      if (!this.isCan || HMI.port.checkUsbUart()) {
        ...
        HMI.port.usbSDK.machineInformation.get
          .dataOfHmi_1(0x05, 0x03)
          .then((res) => {
    ...
    writeSnWithAuth() {
      ...
      if (!this.isCan || HMI.port.checkUsbUart()) {
        ...
        HMI.port.usbSDK.machineInformation.update
          .serialNumber(0x05, 0x03, HMI.port.Core.Util.stringToHex(this.sn))
          .then((res) => {
            //                            console.log(res);
      ...
      if (this.clientCode) {
        if (this.clientCode.length < 25) {
          if (!this.isCan || HMI.port.checkUsbUart()) {
            ...
            HMI.port.usbSDK.machineInformation.update
              .customerNumber(0x05, 0x03, HMI.port.Core.Util.stringToHex(this.clientCode))
              .then((res) => {
                //                                    console.log(res);
                if (res.can === 'NORMAL_ACK') {
                  this.clientCode = ''
    ...
    clickWriteReset() {
      if (this.modelWrite && this.modelWrite.indexOf('DP') > -1) {
        HMI.port.usbSDK.machineInformation.update
          .modelNumber(0x05, 0x03, HMI.port.Core.Util.stringToHex(this.modelWrite.trim()))
          .then((res) => {
            //                            console.log(res);
            if (res.can === 'NORMAL_ACK') {
              this.modelWrite = ''
      ...
      if (this.hardwareWrite && this.hardwareWrite.indexOf('DP') > -1) {
        HMI.port.usbSDK.machineInformation.update
          .hardwareVersionNumber(0x05, 0x03, HMI.port.Core.Util.stringToHex(this.hardwareWrite.trim()))
          .then((res) => {
            //                            console.log(res);
            if (res.can === 'NORMAL_ACK') {
              this.hardwareWrite = ''
              ...
              HMI.port.usbSDK.machineInformation.get
                .hardwareVersionNumber(0x05, 0x03)
                .then((res) => {
    ...
    clearServiceMileageCan() {
      ...
      HMI.port.usbSDK.machineInformation.update
        .dataOfHmi_2(0x05, 0x03, '0000000000')
        .then((res) => {
          //                        console.log(res);
          this.$message({
    ...
    setTotalMileageCanAuth() {
      let buf = Buffer.from(('000000' + parseInt(this.setTotalMileage).toString(16)).slice(-6), 'Hex')
        .reverse()
        .toString('Hex')
      HMI.port.usbSDK.machineInformation.update
        .clear_Hmi_Total_Mileage(0x05, 0x03, buf)
        .then((res) => {
          //                        console.log(res);
          if (res.can === 'NORMAL_ACK') {
            let data = {
    ...
    setSingleMileageCan() {
      ...
      if (typeof single === 'number') {
        let buf = Buffer.from(('000000' + single.toString(16)).slice(-6), 'Hex')
          .reverse()
          .toString('Hex')
        HMI.port.usbSDK.machineInformation.update
          .clear_Hmi_Single_Mileage(0x05, 0x03, buf)
          .then((res) => {
            console.log(res)
            if (res.can === 'NORMAL_ACK') {
              HMI.port.usbSDK.machineInformation.get
    ...
    keyCardStart6501() {
      ...
      if (!this.isCan || HMI.port.checkUsbUart()) {
        ...
        HMI.port.getListenData().subscribe((res) => {
          ...
          if (res.data.code === '65' && res.data.subCode === '01' && res.data.targetCode === 5 && res.data.can === 'NORMAL_ACK') {
            this.$message({
              message: this.$t('pairCardSuccess') + ' ' + parseInt(res.data.data.substr(0, 2), 16),
              type: 'success'
            })
        ...
        HMI.port.usbSDK.machineInformation.update
          .set_Hmi_Card_6501(0x05, 0x03, '01')
          .then((res) => {
            if (res.can === 'NORMAL_ACK') {
              this.$message({
                message: this.$t('pairCardIn'),
    ...
    setTimeCAN() {
      ...
      HMI.port.usbSDK.machineInformation.update
        .set_Hmi_Time(0x05, 0x03, Buffer.from([hour, minute, second]).toString('Hex'))
        .then((res) => {
          //                        console.log(res);
          //                        can: "NORMAL_ACK"
          //                        canCode: 2
    ...
    readSpLevel() {
      ...
      HMI.port.getListenData().subscribe((res) => {
        ...
        if (res.data.code === '63' && res.data.subCode === '08') {
          let result_4 = Buffer.from(res.data.data, 'hex')
          if (result_4.length >= 1) {
            if (result_4[0] === 0) {
              this.spLevel = '空档'
            } else if (result_4[0] === 1) {
              this.spLevel = '前进档'
            } else if (result_4[0] === 2) {
              this.spLevel = '后退档'
            }
          }
        }
    ...
    writeManufacturer() {
      HMI.port.usbSDK.machineInformation.update
        .manufacturer(0x05, 0x03, HMI.port.Core.Util.stringToHex(this.manufacturer.trim()))
        .then((res) => {
          if (res.can === 'NORMAL_ACK') {
            this.$message({
    ...
    writeErrorCode() {
      HMI.port.usbSDK.machineInformation.update
        .errorCode(0x05, 0x03, HMI.port.Core.Util.stringToHex(this.errorCode.trim()))
        .then((res) => {
          if (res.can === 'NORMAL_ACK') {
            this.$message({
              message: this.$t('writeSuccess'),
    ...
    writeBootLoad() {
      HMI.port.usbSDK.machineInformation.update
        .bootLoad(0x05, 0x03, HMI.port.Core.Util.stringToHex(this.bootLoad.trim()))
        .then((res) => {
          if (res.can === 'NORMAL_ACK') {
            this.$message({
              message: this.$t('writeSuccess'),
