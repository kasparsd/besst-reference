  ...
  <div class="update">
    <div class="update-content">
        ...
        <div class="left">
          <div class="item">
            ...
            <el-select v-model="componentType" @change="changeComp" class="input" :placeholder="$t('cNam.select')">
              <el-option v-for="item in optionsCom" :key="item.type" :label="item.label" :value="item.type"> </el-option>
            </el-select>
          </div>
          ...
          <div class="file">
            <label class="label" :style="a" for="uploadFile" ref="fileToU">{{ $t('fileToUpdate') }} </label>
            <br />
          ...
          <div class="btns pull-right" v-if="org.type === Core.Const.ORG_TYPE.COMPONENT">
            <el-button @click="startUpdate" type="preview">{{ $t('update') }}</el-button>
          </div>
          <div class="btns pull-right" v-else>
            <el-button @click="startUpdate" type="preview" :disabled="componentType === ''">{{ $t('update') }}</el-button>
          </div>
        </div>
        <div class="right">
      ...
      <div class="content">
        ...
        <div class="step">
          <div>
            <span class="no">1:</span><span class="stepText"> {{ $t('cNam.updateStep1') }} </span>
          </div>
          <div>
            <span class="no">2:</span><span class="stepText"> {{ $t('cNam.updateStep2') }} </span>
          </div>
          <div>
            <span class="no">3:</span><span class="stepText"> {{ $t('cNam.updateStep3') }} </span>
          </div>
          <div>
            <span class="no">4:</span><span class="stepText"> {{ $t('cNam.updateStep4') }} </span>
          </div>
          <div>
            <span class="no">5:</span><span class="stepText"> {{ $t('cNam.updateStep5') }} </span>
          </div>
          <div>
            <span class="no">6:</span><span class="stepText"> {{ $t('cNam.updateStep6') }} </span>
          </div>
        </div>
      </div>
    </div>
    ...
    <el-dialog :title="$t('cNam.operationTip')" custom-class="tip-dialog" :visible.sync="dialogTipVisible">
      <div>
        ...
        <div class="txt">
          <span class="itemName">{{ $t('update') }}</span>
          <span>{{ $t('cNam.txtTitleUpt') }}</span>
        </div>
...
export default {
  ...
  mounted() {
    ...
    if (this.lang == 'zh' || this.lang == 'en' || this.lang == 'fr') {
      this.a = { width: '220px' }
    }
  ...
  methods: {
    ...
    checkBoxUpdate() {
      ...
      let res = Update.port.usbSDK.client.events.on('firmware-update-completed-progress', (e) => {
        ...
        try {
          // 增加固件超过512k溢出处理进度条问题
          if (e.progress === 0) {
            _tmp_last_progress = 0
          }
          let a = _tmp_last_progress
          if (_tmp_last_progress > e.progress) {
            e.progress += _tmp_last_progress
          } else {
            _tmp_last_progress = e.progress
          }

          let b = e.progress
          let rate = Math.ceil((e.progress / vm.fileBufferLength) * 100)
    ...
    getFirmwareBySn() {
      ...
      Core.Api.Firmware.getHistoryBySn(this.sn).then((res) => {
