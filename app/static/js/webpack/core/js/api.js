...
function Api() {
  ...
  const apiList = {
    ...
    Brand: {
      ...
      switchInvitationType: ['invite/editType', 'id', 'type'],
    ...
    orgAudit: {
      save: ['orgAudit/selectAllOrgAudit', ''],
      reviewer: ['orgAudit/reviewer', ''],
      turnDown: ['orgAudit/turnDown', ''],
      selectAllOrgAudit: ['orgAudit/selectAllOrgAudit', ''],
      selectAll: ['orgAudit/selectAll', ''],
      delete: ['orgAudit/delete', ''],
      selectApplicantRecord: ['orgAudit/selectApplicantRecord', '']
    },
    Dealer: {
      orderList: ['dealer/sale-order-list', 'page'],
      OrderSave: [
        'dealer/sale-order-save',
        'order_id',
        'customer_name',
        'customer_gender',
        'customer_email',
    ...
    Order: {
      ...
      //查询要未完成的要删除的订单
      getNoProducedOrderList: ['order/production-order/list-delete', 'start_time', 'end_time'],
      deleteNoProducedOrderList: ['order/production-order/batch_delete', 'list']
    },
    ProgramBrush: {
      // 订单刷程序逻辑
      // 刷完之后保存, 可以重复保存
      saveBrush: [
        'programBrush/saveProgramBrush',
        'category_id',
        'category_root_id',
        'item_id',
        'part_no',
    ...
    Vehicle: {
      ...
      exportBikeList: ['vehicle/export-for-brand-override', 'brand_org_id', 'start_time', 'end_time']
    },
    Component: {
      saveComponent: ['component/save', 'id', 'order_id', 'sn', 'qc', 'test_data', 'produce_time', 'item_id', 'customer_sn', 'imei', 'mac'],
    ...
    Ticket: {
      createTicket: [
        ...
        'target_id',
        'target_sn',
        'service_center_org_id',
        'img',
        'video',
        'test_data'
      ],
      createTicketWithOrgId: [
        'ticket/create-with-org-id',
        'category',
        'title',
        'description',
        'target_id',
    ...
    // 防伪码
    Security: {
      findSecurityList: ['security/all', 'code', 'page', 'size', 'start_time', 'end_time'],
      deleteSecurity: ['security/delete', 'security_code'],
      editSecurity: ['security/replace', 'new_security_code', 'old_security_code'], // 编辑
      addSecurity: ['security/save', 'security_code', 'sn'], // 增加
      selectSecurity: ['security/select-security-code', 'security_code'], // 页面访问
      uploadExcel: '/security/uploadExcel' // 上传
    },
    FileError: {
  ...
  function post(api, ...args) {
    ...
    return new Promise((resolve, reject) => {
      Vue.http
        .post(url, transformObjectToUrlencodedData(body), {
          headers: headers
        })
        .then(
          (response) => {
    ...
    if (!message) {
      ...
      switch (lan) {
        case 'en':
          ...
          break
        case 'zh':
          message = '服务器没有响应。请检查网络或者联系besst@bafang-e.com'
          break
        case 'nl':
        ...
        case 'de':
          message =
            'Der Fernzugriff auf den Server ist nicht erreichbar oder versuchen später noch einmal. Sie können auch Email an besst@bafang-e.com senden.'
          break
        case 'fr':
          message = 'Unable to connect to remote service. Please check your network or contact besst@bafang-e.com'
          break
  ...
  function postServers(url, body = {}, params = {}) {
    return new Promise((resolve, reject) => {
      ...
      params = Object.assign(params, _params)
      Vue.http
        .post(Const.NET.END_POINT + url, body, {
          params: params
        })
        .then(
          (res) => {
  ...
  function putServers(url, body = {}, params = {}) {
    return new Promise((resolve, reject) => {
      ...
      params = Object.assign(params, _params)
      Vue.http
        .put(Const.NET.END_POINT + url, body, {
          params: params
        })
        .then(
          (res) => {
  ...
  function deleteServers(url) {
    return new Promise((resolve, reject) => {
      const token = Core.Data.getToken() || ''
      Vue.http
        .delete(Const.NET.END_POINT + url, {
          params: {
            token: token
          }
        })
