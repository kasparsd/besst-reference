<template>
  <div class="mechanic-detect">
                ...
                <ul>
                  <li>
                    ...
                    <el-input
                      ...
                      @blur="onBlurSn(hmiInfo.sn)"
                    ></el-input>
                    <el-button
                      type="delete"
                      class="title-select"
                      @click.native.stop="showErrorSelectDialog('hmi')"
                      >{{ $t("selectError") }}
                    </el-button>
                  </li>
                  <li>
                    <img
                      class="icon"
                      src="../../assets/images/icon/icon-controller.png"
                    />
                    ...
                    <el-input
                      ...
                      :placeholder="$t('sn')"
                      @click.native.stop="stopPrevent"
                      @blur="onBlurSn(controllerInfo.sn)"
                    ></el-input>
                    <el-button
                      type="delete"
                      class="title-select"
                      @click.native.stop="showErrorSelectDialog('controller')"
                      >{{ $t("selectError") }}
                    </el-button>
                  </li>
                  <li>
                  ...
                  <li>
                    ...
                    <el-input
                      ...
                      :placeholder="$t('sn')"
                      @click.native.stop="stopPrevent"
                      @blur="onBlurSn(motorInfo.sn)"
                    ></el-input>
                    <el-button
                      type="delete"
                      class="title-select"
                      @click.native.stop="showErrorSelectDialog('motor')"
                      >{{ $t("selectError") }}
                    </el-button>
                  </li>
                  <li>
                    ...
                    <el-input
                      ...
                      :placeholder="$t('sn')"
                      @click.native.stop="stopPrevent"
                      @blur="onBlurSn(pedalSensorInfo.sn)"
                    ></el-input>
                    <el-button
                      type="delete"
                      class="title-select"
              ...
              <table class="table-content" border="0" cellspacing="0" cellpadding="0">
                ...
                <tbody>
                  <tr v-for="item in detectList">
                    ...
                    <td>{{ item.f || "/" }}</td>
                    <td>
                      <a class="btn-underline" @click.stop="goDetectEditView(item)"
                        >Edit</a
                      >
                      <a
                        class="btn-underline"
    ...
    <el-dialog
      :title="$t('selectError')"
      :visible.sync="dialogErrorSelectVisible"
      custom-class="error-select-dialog"
    >
      <div class="content">
        <el-checkbox-group v-model="errorList">
          <el-checkbox
            v-for="error in selectErrorList"
            :key="error.label"
            :label="error.label"
          ></el-checkbox>
        </el-checkbox-group>
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="dialogErrorSelectVisible = false">{{
          $t("cancel")
        }}</el-button>
      </span>
    </el-dialog>
...
export default {
  ...
  i18n: {
    messages: {
      fr: FR.Components.Diagnosis.diagnosisNormal,
  ...
  methods: {
    ...
    showErrorSelectDialog(type) {
      ...
      switch (type) {
        ...
        case "controller":
          ...
          this.errorList = this.controllerInfo.error
            ? this.controllerInfo.error.split(", ")
            : [];
          break;
        case "battery":
          this.selectErrorList = this.batteryDefectOptions;
          this.errorList = this.batteryInfo.error
            ? this.batteryInfo.error.split(", ")
            : [];
          break;
        case "motor":
        ...
        case "pedalSensor":
          ...
          this.errorList = this.pedalSensorInfo.error
            ? this.pedalSensorInfo.error.split(", ")
            : [];
          break;
        case "rpmSensor":
          this.selectErrorList = this.defectOptions;
          this.errorList = this.rpmSensorInfo.error
            ? this.rpmSensorInfo.error.split(", ")
            : [];
          break;
        default:
          break;
    ...
    getVehicle() {
      ...
      Core.Api.Vehicle.vehicleDetailBySn(this.vehicleSn, this.org.id).then((res) => {
