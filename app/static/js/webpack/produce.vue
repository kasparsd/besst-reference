<template>
  <div class="assembler-produce">
    <div class="header">
      ...
      <div class="btn-group-inline">
        <div class="" v-if="!usbHID">
          ...
          <el-select v-model="com" @change="changeCom" :placeholder="$t('cNam.select')" @click.native="getComList">
      ...
      <div class="content">
        ...
        <div class="info read">
          ...
          <div class="search-item">
            ...
            <el-input class="scan-input" v-model="vehicleInfo.vehicleSn" :placeholder="$t('sn')" @blur="blurVehicleSN" :maxlength="45"></el-input>
          </div>
          <div class="search-item">
            ...
            <el-collapse-item name="2">
              <template slot="title">
                ...
                <el-input
                  ...
                  @blur="blurHMISN"
                ></el-input>
                <el-input v-if="hmiInfo.customer_sn" class="input-customer-sn" v-model="hmiInfo.customer_sn" placeholder="Customer SN"></el-input>
                <div class="title-solution" v-if="result.hmi !== '' && result.hmi !== 'OK' && result.hmi !== ''">
                  <el-button type="text" @click.native.stop="dialogSolutionVisible = true">
                    {{ $t('solution') }}
                  </el-button>
                </div>
                <vue-loading class="title-loading" type="spin" color="#E26829" v-if="showLoading.hmi || blur.hmi"></vue-loading>
                <el-progress
                  v-show="showPercentage.hmi"
                  :text-inside="true"
                  :stroke-width="18"
                  :percentage="percentage.hmi"
                  status="success"
                  class="title-progress"
                ></el-progress>
                <el-button type="primary" v-if="updateFlag['hmi']" @click.stop="showHwDialog('hmi')">{{ $t('update') }}</el-button>
              </template>
              <div class="info-list-box">
                <div class="info-list">
                  <div class="col-left">
                    <el-button @click="readBase(HMI)" class="btn-connect" type="primary" :disabled="isRead">
                      {{ $t('cNam.read') }}
                    </el-button>
                    ...
                    <ul class="list-group">
                      ...
                      <li class="list-group-item" v-if="hmiInfo.customer_sn">
                        <span>Customer SN</span>
                        <span>{{ hmiInfo.customer_sn || '-' }}</span>
                      </li>
                      <li class="list-group-item">
            ...
            <el-collapse-item name="3">
              <template slot="title">
                ...
                <el-input
                  ...
                  :rows="1"
                  resize="none"
                  v-model="controllerInfo.sn"
                  :placeholder="$t('sn')"
                  @click.native.stop="stopPrevent"
                  @blur="blurControllerSN"
                ></el-input>
                <el-input
                  v-if="controllerInfo.customer_sn"
                  class="input-customer-sn"
                  v-model="controllerInfo.customer_sn"
                  placeholder="Customer SN"
                ></el-input>
                <div class="title-solution" v-if="result.controller !== '' && result.controller !== 'OK' && result.controller !== ''">
                  <el-button type="text" @click.native.stop="showSolutionDialog">
                    {{ $t('solution') }}
                  </el-button>
                </div>
                <vue-loading class="title-loading" type="spin" color="#E26829" v-if="showLoading.controller || blur.ctrl"></vue-loading>
                <el-progress
                  v-show="showPercentage.controller"
                  :text-inside="true"
                  :stroke-width="18"
                  :percentage="percentage.controller"
                  status="success"
                  class="title-progress"
                ></el-progress>

                <el-button type="primary" v-if="updateFlag['ctrl']" @click.stop="showHwDialog('ctrl')">{{ $t('update') }} </el-button>
              </template>
              <div class="info-list-box">
                <div class="info-list">
                  <div class="col-left">
                    <el-button @click="readBase(Controller)" class="btn-connect" type="primary" :disabled="isRead">
                      {{ $t('cNam.read') }}
                    </el-button>
                    ...
                    <ul class="list-group">
                      ...
                      <li class="list-group-item" v-if="controllerInfo.customer_sn">
                        <span>Customer SN</span>
                        <span>{{ controllerInfo.customer_sn || '-' }}</span>
                      </li>
                      <li class="list-group-item">
                        <span>{{ $t('cNam.model') }}</span>
            ...
            <el-collapse-item name="4">
              <template slot="title">
                ...
                <el-input
                  ...
                  :rows="1"
                  resize="none"
                  v-model="batteryInfo.sn"
                  :placeholder="$t('sn')"
                  @click.native.stop="stopPrevent"
                  @blur="blurBatterySN"
                ></el-input>
                <el-input
                  v-if="batteryInfo.customer_sn"
                  class="input-customer-sn"
                  v-model="batteryInfo.customer_sn"
                  placeholder="Customer SN"
                ></el-input>
                <div class="title-solution" v-if="result.battery !== '' && result.battery !== 'OK' && result.battery !== ''">
                  <el-button type="text" @click.native.stop="showSolutionDialog">
                    {{ $t('solution') }}
                  </el-button>
                </div>
                <vue-loading class="title-loading" type="spin" color="#E26829" v-if="showLoading.battery || blur.bat"></vue-loading>
                <el-progress
                  v-show="showPercentage.battery"
                  :text-inside="true"
                  :stroke-width="18"
              ...
              <div class="info-list-box">
                <div class="info-list">
                  <div class="col-left">
                    <el-button @click="readBase(Battery)" class="btn-connect" :disabled="isRead" type="primary">
                      {{ $t('cNam.read') }}
                    </el-button>
                    ...
                    <ul class="list-group">
                      ...
                      <li class="list-group-item" v-if="batteryInfo.customer_sn">
                        <span>Customer SN</span>
                        <span>{{ batteryInfo.customer_sn || '-' }}</span>
                      </li>
                      <li class="list-group-item">
                        <span>{{ $t('cNam.softwareVersion') }}</span>
          ...
          <el-collapse v-model="activeNames">
            ...
            <el-collapse-item name="5">
              <template slot="title">
                ...
                <el-input
                  ...
                  :rows="1"
                  resize="none"
                  v-model="motorInfo.sn"
                  :placeholder="$t('sn')"
                  @click.native.stop="stopPrevent"
                  @blur="blurMotorSN"
                ></el-input>
                <el-input v-if="motorInfo.customer_sn" class="input-customer-sn" v-model="motorInfo.customer_sn" placeholder="Customer SN"></el-input>
                <div class="title-solution" v-if="result.motor !== '' && result.motor !== 'OK' && result.motor !== ''">
                  <el-button type="text" @click.native.stop="showSolutionDialog">
                    {{ $t('solution') }}
                  </el-button>
                </div>
                <vue-loading class="title-loading" type="spin" color="#E26829" v-if="blur.motor"></vue-loading>
              </template>
              <div class="info-list-box">
                <div class="info-list">
                  <div class="col-left">
                    <ul class="list-group mr-tpo-0">
                      ...
                      <li class="list-group-item" v-if="motorInfo.customer_sn">
                        <span>Customer SN</span>
                        <span>{{ motorInfo.customer_sn || '-' }}</span>
                      </li>
                      <li class="list-group-item">
            ...
            <el-collapse-item name="6">
              <template slot="title">
                ...
                <el-input
                  ...
                  :rows="1"
                  resize="none"
                  v-model="pedalSensorInfo.sn"
                  :placeholder="$t('sn')"
                  @click.native.stop="stopPrevent"
                  @blur="blurSensorSN"
                ></el-input>
                <el-input
                  v-if="pedalSensorInfo.customer_sn"
                  class="input-customer-sn"
                  v-model="pedalSensorInfo.customer_sn"
                  placeholder="Customer SN"
                ></el-input>
                <div class="title-solution" v-if="result.pedalSensor !== '' && result.pedalSensor !== 'OK' && result.pedalSensor !== ''">
                  <el-button type="text" @click.native.stop="dialogSolutionVisible = true">
                    {{ $t('solution') }}
                  </el-button>
                </div>
                <vue-loading class="title-loading" type="spin" color="#E26829" v-if="(showLoading.sensor && isCan) || blur.sensor"></vue-loading>
                <el-progress
                  v-show="showPercentage.sensor && isCan"
                  :text-inside="true"
                  :stroke-width="18"
                  :percentage="percentage.sensor"
                  status="success"
                  class="title-progress"
                ></el-progress>
                <el-button type="primary" v-if="updateFlag['sensor'] && isCan" @click.stop="showHwDialog('sensor')">{{ $t('update') }}</el-button>
              </template>
              <div class="info-list-box">
                <div class="info-list">
                  <div class="col-left">
                    <ul class="list-group mr-tpo-0">
                      ...
                      <li class="list-group-item" v-if="pedalSensorInfo.customer_sn">
                        <span>Customer SN</span>
                        <span>{{ pedalSensorInfo.customer_sn || '-' }}</span>
                      </li>
                      <li class="list-group-item">
                        <span>{{ $t('cNam.softwareVersion') }}</span>
            ...
            <el-collapse-item name="7">
              <template slot="title">
                ...
                <el-input class="input-sn" v-model="vehicleInfo.key" :placeholder="$t('cNam.keyCard')" :maxlength="45"></el-input>
              </template>
            </el-collapse-item>
            <!--Tracking code-->
    ...
    <!--视频弹框-->
    <el-dialog :title="$t('connectTip')" custom-class="video-dialog" :visible.sync="dialogVideoVisible">
    ...
    <el-dialog :title="$t('historyDialog.title')" :visible.sync="historyDialogVisible" class="history-dialog">
      ...
      <div class="clear-float">
        <span> {{ $t('cNam.updateMsgTip') }}</span
        ><el-button @click="resetUpdate" class="pull-right">{{ $t('reset') }}</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  i18n: {
    messages: {
      fr: FR.Assembler.AssemblerOrderProduce,
  ...
  methods: {
          ...
          if (!vm.isCan || HMI.port.checkUsbUart()) {
            ...
            HMI.getBus().subscribe((res) => {
              ...
              if (res.data.hmi.hasOwnProperty('sn')) {
                ...
                if (vm.hmiInfo.sn !== '') {
                  // 查询当前sn是否存在客户sn
                  CustomerSNService.findCustomerSN(vm.hmiInfo.sn).then(({ data }) => {
                    const { customerSn } = data.data
                    vm.hmiInfo.customer_sn = customerSn || ''
                  })
                }
                vm.blurHMISN()
              }
              ...
              vm.percentage.hmi = vm.getPercentage([
                vm.hmiInfo.sn,
                vm.hmiInfo.model,
                vm.hmiInfo.version,
                vm.hmiInfo.hardVersion,
                vm.hmiInfo.wheelDiameter,
            ...
            HMI.port.usbSDK.machineInformation.get.dataOfHmi_1(0x05, 0x03).then((res) => {
          ...
          if (!vm.isCan || Controller.port.checkUsbUart()) {
            ...
            Controller.getBus().subscribe((res) => {
              ...
              if (res.data.ctrl.hasOwnProperty('sn')) {
                ...
                if (vm.controllerInfo.sn !== '') {
                  // 查询当前sn是否存在客户sn
                  CustomerSNService.findCustomerSN(vm.controllerInfo.sn).then(({ data }) => {
                    const { customerSn } = data.data
                    vm.controllerInfo.customer_sn = customerSn || ''
                  })
                }
                vm.blurControllerSN()
              }
              ...
              this.percentage.controller = this.getPercentage([
                this.controllerInfo.sn,
                this.controllerInfo.model,
                this.controllerInfo.version,
                this.controllerInfo.manufacturer,
                this.controllerInfo.nmlVolt,
                this.controllerInfo.maxCurrent,
                this.controllerInfo.hardVersion
              ])
            ...
            Controller.port.getUsbBasic(0x02).subscribe((res) => {
              ...
              if (res.data.hasOwnProperty('sn')) {
                ...
                if (this.controllerInfo.sn !== '') {
                  // 查询当前sn是否存在客户sn
                  CustomerSNService.findCustomerSN(this.controllerInfo.sn).then(({ data }) => {
                    const { customerSn } = data.data
                    this.controllerInfo.customer_sn = customerSn || ''
                  })
                }
          ...
          if (!vm.isCan || Battery.port.checkUsbUart()) {
            ...
            Battery.getBus().subscribe((res) => {
              ...
              if (res.data.battery.hasOwnProperty('sn')) {
                ...
                if (this.batteryInfo.sn !== '') {
                  // 查询当前sn是否存在客户sn
                  CustomerSNService.findCustomerSN(this.batteryInfo.sn).then(({ data }) => {
                    const { customerSn } = data.data
                    this.batteryInfo.customer_sn = customerSn || ''
                  })
                }
                this.blurBatterySN()
              }
              ...
              this.percentage.battery = this.getPercentage([
                this.batteryInfo.sn,
                this.batteryInfo.model,
                this.batteryInfo.version,
                this.batteryInfo.hardVersion,
                this.batteryInfo.soc,
                this.batteryInfo.soh,
                this.batteryInfo.fullChargerCapacity,
                this.batteryInfo.leftCapacity,
                this.batteryInfo.temperature,
                this.batteryInfo.current,
                this.batteryInfo.loopTime,
                this.batteryInfo.maxChargeInterval
              ])
          ...
          Sensor.port.getUsbBasic(0x01).subscribe((res) => {
            ...
            if (res.data.hasOwnProperty('sn')) {
              ...
              if (this.pedalSensorInfo.sn !== '') {
                // 查询当前sn是否存在客户sn
                CustomerSNService.findCustomerSN(this.pedalSensorInfo.sn).then(({ data }) => {
                  const { customerSn } = data.data
                  this.pedalSensorInfo.customer_sn = customerSn || ''
                })
              }
              this.blurSensorSN()
            }
            ...
            this.percentage.sensor = this.getPercentage([
              this.pedalSensorInfo.sn,
              this.pedalSensorInfo.model,
              this.batteryInfo.version,
              this.pedalSensorInfo.hardVersion
            ])
    ...
    getFirmwareBySn(sn) {
      ...
      Core.Api.Firmware.getHistoryBySn(sn).then((res) => {
        ...
        this.HWRecord.length ? (this.historyDialogVisible = true) : this.$message.warning('no firmware')
      })
    },
    ...
    doUpdate(data) {
      ...
      if (vm.usbHID) {
        ...
        setTimeout(() => {
          //                        console.log(bus, 'bus');
          Update.sendPackage(data).subscribe((res) => {
            if (res.type === 'close' || res.type === 'error') {
              this.isConnected = false
              this.com = ''
            }
  ...
  watch: {
      ...
      handler: function (newValue, oldValue) {
        ...
        if (!vm.scannerTimeout) {
          vm.scannerTimeout = setTimeout(function () {
            if (vm.isScanner) {
              // 判断是否是客户sn规则
              if (vm.hmiInfo.sn.indexOf('\n') > -1) {
                let array = vm.hmiInfo.sn.split('\n')
                if (array.length < 4 && array[1] !== '') {
                  vm.hmiInfo.customer_sn = array[0]
                  vm.hmiInfo.sn = array[1]
                }
              }
    ...
    'controllerInfo.sn': {
      handler: function (newValue, oldValue) {
        ...
        this.scannerTimeout = setTimeout(() => {
          if (this.controllerInfo.sn.indexOf('\n') > -1) {
            let array = this.controllerInfo.sn.split('\n')
            if (array.length < 4 && array[1] !== '') {
              this.controllerInfo.customer_sn = array[0]
              this.controllerInfo.sn = array[1]
            }
          }
          this.controllerInfo.sn = this.controllerInfo.sn.replace(/\s+/g, '')
    ...
    'batteryInfo.sn': {
      handler: function (newValue, oldValue) {
        if (this.scannerTimeout) clearTimeout(this.scannerTimeout)
        this.scannerTimeout = setTimeout(() => {
          if (this.batteryInfo.sn.indexOf('\n') > -1) {
            let array = this.batteryInfo.sn.split('\n')
            if (array.length < 4 && array[1] !== '') {
              this.batteryInfo.customer_sn = array[0]
              this.batteryInfo.sn = array[1]
            }
          }
          this.batteryInfo.sn = this.batteryInfo.sn.replace(/\s+/g, '')
          clearTimeout(this.scannerTimeout)
          this.scannerTimeout = undefined
        }, 1000)
        // 判断是否是客户sn规则
    ...
    'motorInfo.sn': {
      handler: function (newValue, oldValue) {
        if (this.scannerTimeout) clearTimeout(this.scannerTimeout)
        this.scannerTimeout = setTimeout(() => {
          if (this.motorInfo.sn.indexOf('\n') > -1) {
            let array = this.motorInfo.sn.split('\n')
            if (array.length < 4 && array[1] !== '') {
              this.motorInfo.customer_sn = array[0]
              this.motorInfo.sn = array[1]
            }
          }
          this.motorInfo.sn = this.motorInfo.sn.replace(/\s+/g, '')
          clearTimeout(this.scannerTimeout)
          this.scannerTimeout = undefined
        }, 1000)
      },
      ...
      handler: function (newValue, oldValue) {
        ...
        if (!vm.scannerTimeout) {
          vm.scannerTimeout = setTimeout(function () {
            if (vm.isScanner) {
              // 判断是否是客户sn规则
              if (vm.pedalSensorInfo.sn.indexOf('\n') > -1) {
                let array = vm.pedalSensorInfo.sn.split('\n')
                if (array.length < 4 && array[1] !== '') {
                  vm.pedalSensorInfo.customer_sn = array[0]
                  vm.pedalSensorInfo.sn = array[1]
                }
              }
              vm.pedalSensorInfo.sn = vm.pedalSensorInfo.sn.replace(/\s+/g, '')
              // 如果序列号中匹配到特定字符，自动显示在对应的输入框中
              if (vm.pedalSensorInfo.sn.indexOf('SR') > -1) {
                let array = vm.pedalSensorInfo.sn.split('SR')
                if (array.length > 1) {
                  vm.pedalSensorInfo.sn = 'SR' + array[array.length - 1]
                }
              }
            }
            clearTimeout(vm.scannerTimeout)
