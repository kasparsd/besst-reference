...
const requireComponent = require.context('./components', true, /(EV).*\.vue$/)
requireComponent.keys().forEach((fileName) => {
  const componentConfig = requireComponent(fileName)

  function getComponentNameFromFile() {
    const split = fileName.split('/')
    return split[split.length - 1].replace('.vue', '')
  }
  const componentName = componentConfig.name || getComponentNameFromFile()
